...
* dynamic priority
* 매 task의 종료 시 다시 스케줄링하여 시간 내의 데드라인이 가장 근접한 태스크를 스케줄링
...
* critical section에서 발생
  * critical sectino이란 shared data가 접근되는 code의 segment로, 여러 태스크가 동시에 접근할 경우 race condition이 발생할 수 있으므로 semaphore를 통해 보호해야함
* RMS라는 가정 하에 발생
