...
> #### Sigaction 함수(시그널 함수 대체)
* 첫번째 인자: 시그널의 정보
* 두번째 인자: 해당 시그널이 발생했을 때 호출할 함수의 정보를 구조체 변수에 담아서 넘겨줌
* 세번째 인자: 이전에 등록되었던 시그널 함수의 포인터를 얻는데 사용되는 인자이지만, 필요없다면 0 전달
<pre><code>
struct sigaction { 
  void (*sa handler)(int); // 시그널 발생시 발생할 함수의 포인터 값
  sigset_t sa_mask; // 블록될 시그널의 집합 
  int sa_flags; // 여러 옵션을 추가(예. 정상 종료한 child만 받게끔..)
}
</code></pre>
참고: 네트워크 실습 시간에는 두번째, 세번째 인자는 0으로 보내줄 것

> 시그널 핸드링을 통한 좀비 프로세스의 소멸
* sigaction 구조체 변수 선언
